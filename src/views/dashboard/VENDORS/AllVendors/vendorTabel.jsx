...
import { Table, Input, Space, Avatar, Button, Badge, Switch, Tooltip } from 'antd';
import { EditOutlined, DeleteOutlined, EyeOutlined } from '@ant-design/icons';
import { useNavigate } from 'react-router-dom';
import useVendors from './hooks/useVendors';
import { convertTimeRange } from '../../../../services/utils/gen_utility';
...
export default function VendorTable() {
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'storeImage',
      render: (img) => <Avatar size={70} src={img} alt="store" />,
      width: 70,
    },
    {
      title: 'Store Name',
    ...
    },
    {
      ...
      render: (number, record) => {
        ...
        const raw = (record.prefix || '+1') + number;
        let formatted = '';
        if (number && number.length === 10) {
          formatted = `${record.prefix || '+1'} (${number.slice(0,3)}) ${number.slice(3,6)} ${number.slice(6)}`;
        } else if (number && number.length === 11 && number.startsWith('1')) {
          formatted = `+1 (${number.slice(1,4)}) ${number.slice(4,7)} ${number.slice(7)}`;
        } else {
          formatted = `${record.prefix || '+1'} ${number}`;
        }
        return <span style={{ minWidth: 120, display: 'inline-block' }}>{formatted}</span>;
      },
    },
    {
      title: 'Email',
